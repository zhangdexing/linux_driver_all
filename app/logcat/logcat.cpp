...
int initBlackList(const std::string& path)
{
    std::string file;
    if (!android::base::ReadFileToString(path, &file))
    {
        return -1;
    }
    BlackList = android::base::Split(file, "\n");
    return 1;
}
bool isInBlackList(const char *tag)
{
    if (BlackList.size() < 1)
        return false;
for (const auto & line : BlackList)
    {
        if (!strcmp(line.c_str(),tag))
        {
            return true;
        }
    }
...
static int g_whiteFd = -1;
int initWhiteList(const std::string& path)
{
    std::string file;
    if (!android::base::ReadFileToString(path, &file))
    {
    ...
    WhiteList = android::base::Split(file, "\n");
    return 1;
}

bool isInWhiteList(const char *tag)
{
    if (WhiteList.size() < 1)
        return false;
