...
int ParseMultiStreamConfig(unsigned char *pConfig,  struct MultiStreamCfg *Cfg_Desc)
{
    ...
    Cfg_Desc->MS_Cap = (struct MultiStreamCap*)malloc(Cfg_Desc->NumStreams * sizeof(struct MultiStreamCap));
 ...
 int ParseCapability(unsigned char *pData, int Length, struct CapabilityDescriptor *Cap_Desc)
 {
    ...
    Cap_Desc->Cfg_Desc = (struct MultiStreamCfg*)malloc(Cap_Desc->NumConfigs * sizeof(struct MultiStreamCfg));
    ...
    pDemuxerData = pConfigData;
    Cap_Desc->demuxer_Desc = (struct MultiStreamDemuxer*)malloc(Cap_Desc->NumDemuxers * sizeof(struct MultiStreamDemuxer));
    ...
    pFrameIntervalData = pDemuxerData;
    Cap_Desc->FrameInt_Desc = (struct MultiStreamFrameInterval*)malloc(Cap_Desc->NumFrameIntervals * sizeof(struct MultiStreamFrameInterval));
    for(i = 0; i < Cap_Desc->NumFrameIntervals; i++)
    {
        ...
        Cap_Desc->FrameInt_Desc[i].FPS = (unsigned char*)malloc(Cap_Desc->FrameInt_Desc[i].FPSCount * sizeof(unsigned char));
    ...
    Cap_Desc->Bitrate_Desc = (struct MultiStreamBitrate*)malloc(Cap_Desc->NumBitrate*sizeof(struct MultiStreamBitrate));
