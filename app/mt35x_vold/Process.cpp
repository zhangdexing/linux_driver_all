...
void Process::killProcessesWithOpenFiles(const char *path, int signal) {
    ...
    /*
    *	Fix /mnt/runtime/write/uuid directory umount fail problem
    *	When try umount /mnt/runtime/write/uuid fail, try to check whether the fd is opening /storage/uuid files
    * 	Add by Barton Zhang
    */
    char storagepath[128] = "";
    char* start = strstr(path, "write/");
    if (start != nullptr) {
        strcat(storagepath, "/storage/");
        strcat(storagepath, start + 6);
    }
    ...
    while ((de = readdir(dir))) {
        ...
        if (checkFileDescriptorSymLinks(pid, path, openfile, sizeof(openfile)) ||
            checkFileDescriptorSymLinks(pid, storagepath, openfile, sizeof(openfile))) {
