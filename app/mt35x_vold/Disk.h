...
public:
    ...
    static const unsigned int kMinorBlockFon = 1;/*fat on nand device*/

    // Prevent from receiving 2 disk changed uevents immediately (One is zap-all, and the other is 'format')
    // That will run 'mount' operation twice and make status abnormal. Therefore, we skip the second one.
    bool mSkipDiskChangedUevent;
    bool mShared;
};
