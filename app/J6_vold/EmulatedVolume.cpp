...
EmulatedVolume::EmulatedVolume(const std::string& rawPath, dev_t device,
    ...
    setStorageType(android::vold::VolumeBase::StorageType::kExternalSD);
}
...
status_t EmulatedVolume::doMount() {
    ...
    if ((!isMountpointMounted(mFuseDefault.c_str()) && fs_prepare_dir(mFuseDefault.c_str(), 0700, AID_ROOT, AID_ROOT)) ||
         (!isMountpointMounted(mFuseRead.c_str()) && fs_prepare_dir(mFuseRead.c_str(), 0700, AID_ROOT, AID_ROOT)) ||
         (!isMountpointMounted(mFuseWrite.c_str()) && fs_prepare_dir(mFuseWrite.c_str(), 0700, AID_ROOT, AID_ROOT))) {
