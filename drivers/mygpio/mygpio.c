...
/*	int ret = 0;
	...
	else if(!strcmp("write_read",cmd))
	{
		...
		spi_api_do_write_then_read(2,txbuf, size, rxbuf,4);
	...
	{	
		int i;
		char rxbuf[100];
		ret = spi_api_do_read(2,rxbuf,10);
		for(i=0;i<10;i++)
		{
			debugPrintf("rxbuf[%d]=%02x\n",i,rxbuf[i]);
		}
...
static int __init spi_init(void)
{
	...
	ret = gpio_request(89,"BACKLIGHT_EN");
	if(ret<0)
		debugPrintf("==>gpio 1failed.\n");
	ret = gpio_direction_output(89,0);
	if(ret<0)
		debugPrintf("==>gpio 2failed.\n");
	gpio_set_value(89,0);

	debugPrintf("==>gpio begin.\n");
