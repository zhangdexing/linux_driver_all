...
static int cyttsp4_btn_suspend_attention(struct device *dev)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp4_btn_data *bd = &cd->bd;
...
static int cyttsp4_btn_resume_attention(struct device *dev)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp4_btn_data *bd = &cd->bd;
...
static int cyttsp4_btn_open(struct input_dev *input)
{
	...
	cyttsp4_subscribe_attention_(dev, CY_ATTEN_IRQ, CY_MODULE_BTN,
		cyttsp4_btn_attention, CY_MODE_OPERATIONAL);

	/* set up startup call back */
	cyttsp4_subscribe_attention_(dev, CY_ATTEN_STARTUP, CY_MODULE_BTN,
		cyttsp4_startup_attention, 0);

	/* set up suspend call back */
	cyttsp4_subscribe_attention_(dev, CY_ATTEN_SUSPEND, CY_MODULE_BTN,
		cyttsp4_btn_suspend_attention, 0);

	/* set up resume call back */
	cyttsp4_subscribe_attention_(dev, CY_ATTEN_RESUME, CY_MODULE_BTN,
		cyttsp4_btn_resume_attention, 0);
...
static void cyttsp4_btn_close(struct input_dev *input)
{
	...
	cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_IRQ, CY_MODULE_BTN,
		cyttsp4_btn_attention, CY_MODE_OPERATIONAL);

	cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_STARTUP, CY_MODULE_BTN,
		cyttsp4_startup_attention, 0);

	cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_SUSPEND, CY_MODULE_BTN,
		cyttsp4_btn_suspend_attention, 0);

	cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_RESUME, CY_MODULE_BTN,
		cyttsp4_btn_resume_attention, 0);
