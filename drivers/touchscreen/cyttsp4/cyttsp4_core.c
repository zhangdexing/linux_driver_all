...
static u16 cyttsp4_calc_app_crc(const u8 *data, int size)
{
	int i, j;
	u16 crc = 0xFFFF;
...
static int cyttsp4_load_status_and_touch_regs_(struct cyttsp4_core_data *cd,
		bool optimize)
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
...
static int cyttsp4_handshake_(struct cyttsp4_core_data *cd, u8 mode)
{
...
static int cyttsp4_si_data_offsets_(struct cyttsp4_core_data *cd)
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	int rc;

	rc = cyttsp4_adap_read(cd, CY_REG_BASE, &si->si_data,
		sizeof(si->si_data));
	if (rc < 0) {
...
static int cyttsp4_si_get_cydata_(struct cyttsp4_core_data *cd)
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	void *p;
	int rc;
...
static int cyttsp4_si_get_test_data_(struct cyttsp4_core_data *cd)
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	void *p;
	int rc;
...
static int cyttsp4_si_get_pcfg_data_(struct cyttsp4_core_data *cd)
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	void *p;
	int rc;
...
static int cyttsp4_si_get_opcfg_data_(struct cyttsp4_core_data *cd)
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	int i;
	enum cyttsp4_tch_abs abs;
...
static int cyttsp4_si_get_ddata_(struct cyttsp4_core_data *cd)
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	void *p;
	int rc;
...
static int cyttsp4_si_get_mdata_(struct cyttsp4_core_data *cd)
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	void *p;
	int rc;
...
static int cyttsp4_get_sysinfo_regs_(struct cyttsp4_core_data *cd)
{
	struct cyttsp4_sysinfo *si = &cd->sysinfo;
	int rc;

	rc = cyttsp4_si_data_offsets_(cd);
	if (rc < 0)
		return rc;

	rc = cyttsp4_si_get_cydata_(cd);
	if (rc < 0)
		return rc;

	rc = cyttsp4_si_get_test_data_(cd);
	if (rc < 0)
		return rc;

	rc = cyttsp4_si_get_pcfg_data_(cd);
	if (rc < 0)
		return rc;

	rc = cyttsp4_si_get_opcfg_data_(cd);
	if (rc < 0)
		return rc;

	rc = cyttsp4_si_get_ddata_(cd);
	if (rc < 0)
		return rc;

	rc = cyttsp4_si_get_mdata_(cd);
	if (rc < 0)
		return rc;
	...
	rc = cyttsp4_handshake_(cd, si->si_data.hst_mode);
	if (rc < 0)
...
static void _cyttsp4_queue_startup(struct cyttsp4_core_data *cd)
{
...
static void cyttsp4_postpone_wd_timer(struct cyttsp4_core_data *cd)
{
	if (!CY_WATCHDOG_TIMEOUT)
		return;
...
static void cyttsp4_parse_mode(struct device *dev, u8 mode,
...
static irqreturn_t cyttsp4_irq(int irq, void *handle)
{
	...
	cyttsp4_parse_mode(dev, mode[0], &cur_mode);
	...
	if (cd->mode == CY_MODE_OPERATIONAL) {
		dev_vdbg(dev, "%s: Read status and touch registers\n",
			__func__);
		rc = cyttsp4_load_status_and_touch_regs_(cd, !command_complete);
		if (rc < 0)
...
cyttsp4_irq_handshake:
	...
	cyttsp4_postpone_wd_timer(cd);

	/* handshake the event */
	dev_vdbg(dev, "%s: Handshake mode=0x%02X r=%d\n",
			__func__, mode[0], rc);
	rc = cyttsp4_handshake_(cd, mode[0]);
	if (rc < 0)
	...
	int size)
{
	...
	return cyttsp4_adap_write(cd, addr, buf, size);
}
...
int cyttsp4_read_(struct device *dev, int mode, u16 addr, void *buf, int size)
{
	...
	return cyttsp4_adap_read(cd, addr, buf, size);
}

int cyttsp4_subscribe_attention_(struct device *dev,
...
int cyttsp4_unsubscribe_attention_(struct device *dev,
	enum cyttsp4_atten_type type, char id, int (*func)(struct device *),
	int mode)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
...
int cyttsp4_request_exclusive(struct device *dev, int timeout_ms)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
...
int cyttsp4_release_exclusive(struct device *dev)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
 ...
 */
static int set_mode_(struct cyttsp4_core_data *cd, int new_mode)
{
	u8 new_dev_mode;
	u8 mode;
	long t;
...
static int cyttsp4_wait_cmd_exec(struct cyttsp4_core_data *cd, int timeout_ms)
{
	struct device *dev = cd->dev;
	int rc;
...
static int get_cmd_offs_and_cmd_data_len(struct cyttsp4_core_data *cd,
		...
		size_t param_ret_size, int timeout_ms)
{
	...
	if (rc == -EBUSY) {
		rc = cyttsp4_wait_cmd_exec(cd, CY_COMMAND_COMPLETE_TIMEOUT);
		if (rc)
			return rc;
		mutex_lock(&cd->system_lock);
		rc = _cyttsp4_exec_cmd(cd, mode, cmd_buf, cmd_size);
	 ...
	 */
	rc = cyttsp4_wait_cmd_exec(cd, timeout_ms);
	if (rc < 0)
		return rc;
	...
	rc = cyttsp4_adap_read(cd, cmd_return_ofs, return_buf, param_ret_size);
	if (rc < 0) {
...
static int cyttsp4_watchdog_work_(struct cyttsp4_core_data *cd)
{
...
#else
static int _cyttsp4_watchdog_work(struct cyttsp4_core_data *cd)
{
	u8 mode[2];
	int rc;
...
static int cyttsp4_wait_for_refresh_cycle(struct cyttsp4_core_data *cd,
		int cycle)
{
...
static int _cyttsp4_core_sleep_(struct cyttsp4_core_data *cd)
{
	int rc;
...
static int cyttsp4_core_sleep(struct cyttsp4_core_data *cd)
{
	...
	cyttsp4_wait_for_refresh_cycle(cd, 2);
		...
		int timeout_ms)
{
	...
	cyttsp4_adap_read(cd, CY_REG_BASE, &mode, sizeof(mode));
...
static int _exit_bootloader(struct cyttsp4_core_data *cd)
{
	const u8 *exit_cmd;
	int exit_cmd_size;
	int rc;
	...
	rc = cyttsp4_adap_write(cd, CY_REG_BASE, exit_cmd, exit_cmd_size);
	if (rc < 0)
...
static int _fast_startup(struct cyttsp4_core_data *cd)
{
	struct device *dev = cd->dev;
	int rc;
...
static int cyttsp4_startup_(struct cyttsp4_core_data *cd, bool reset_needed)
{
...
reset:
	...
	/*
	 * reset_needed flag should be set as FALSE only by cyttsp4_probe().
	 */
	if (!reset_needed) {
		rc = cyttsp4_wait_bl_heartbeat(cd);
		if (rc < 0) {
	...
	rc = cyttsp4_get_sysinfo_regs_(cd);
	if (rc < 0) {
...
exit_no_wd:
	...
	/* Required for signal to the TTHE */
	dev_info(cd->dev, "%s: cyttsp4_exit startup rc=%d...\n", __func__, rc);

	return rc;
}

static int cyttsp4_startup(struct cyttsp4_core_data *cd, bool reset_needed)
{
	int rc;
	...
	rc = cyttsp4_startup_(cd, reset_needed);
...
static int cyttsp4_pm_notifier(struct notifier_block *nb,
		unsigned long action, void *data)
{
	struct cyttsp4_core_data *cd = container_of(nb,
			struct cyttsp4_core_data, pm_notifier);
	...
	if (action == PM_SUSPEND_PREPARE) {
		...
		(void) cyttsp4_core_suspend(cd->dev);
	}
		...
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, CY_MAX_PRBUF_SIZE,
		"Driver: %s\nVersion: %s\nDate: %s\n",
		CYTTSP4_CORE_NAME, CY_DRIVER_VERSION, CY_DRIVER_DATE);
}
...
static struct cyttsp4_core_commands _cyttsp4_core_commands = {
	...
	.read = cyttsp4_read_,
	.calc_app_crc = cyttsp4_calc_app_crc,
};
...
{
	struct cyttsp4_core_data *cd =
		container_of(self, struct cyttsp4_core_data, fb_notifier);
...
static void cyttsp4_setup_fb_notifier(struct cyttsp4_core_data *cd)
{
	int rc;
...
static int cyttsp4_setup_irq_gpio(struct cyttsp4_core_data *cd)
{
	struct device *dev = cd->dev;
...
static int cyttsp4_set_max_xfer(struct cyttsp4_core_data *cd)
{
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	rc = cyttsp4_startup(cd, !CY_CORE_DEVICE_RESET_BY_PLATFORM_INIT);
...
#elif defined(CONFIG_FB)
	cyttsp4_setup_fb_notifier(cd);
#endif

#if NEED_SUSPEND_NOTIFIER
	cd->pm_notifier.notifier_call = cyttsp4_pm_notifier;
