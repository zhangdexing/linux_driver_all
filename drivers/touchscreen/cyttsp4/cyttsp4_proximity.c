...
static inline struct cyttsp4_proximity_data *get_prox_data(struct device *dev)
{
		...
		int num_cur_rec)
{
	...
	for (i = 0; i < num_cur_rec; i++) {
		cyttsp4_get_touch_record(pd->dev, i, tch.abs);
...
static int cyttsp4_proximity_attention(struct device *dev)
{
	struct cyttsp4_proximity_data *pd = get_prox_data(dev);
	int rc = 0;
...
static int _cyttsp4_proximity_enable(struct cyttsp4_proximity_data *pd)
{
	...
	rc = cyttsp4_request_exclusive(dev,
	...
	rc = cyttsp4_request_enable_scan_type_(dev, CY_ST_PROXIMITY);
	if (rc < 0) {
	...
	cyttsp4_subscribe_attention_(dev, CY_ATTEN_IRQ, CY_MODULE_PROX,
		cyttsp4_proximity_attention, CY_MODE_OPERATIONAL);

	/* set up startup call back */
	cyttsp4_subscribe_attention_(dev, CY_ATTEN_STARTUP, CY_MODULE_PROX,
		cyttsp4_startup_attention, 0);

exit_release:
	cyttsp4_release_exclusive(dev);
exit:
	return rc;
}
		...
		bool force)
{
	...
	rc = cyttsp4_request_disable_scan_type_(dev, CY_ST_PROXIMITY);
	if (rc < 0) {
...
exit:
	if (!rc || force) {
		cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_IRQ,
			CY_MODULE_PROX, cyttsp4_proximity_attention,
			CY_MODE_OPERATIONAL);

		cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_STARTUP,
			CY_MODULE_PROX, cyttsp4_startup_attention, 0);
	}
...
static int cyttsp4_setup_input_device_and_sysfs(struct device *dev)
{
	struct cyttsp4_proximity_data *pd = get_prox_data(dev);
...
static int cyttsp4_setup_input_attention(struct device *dev)
{
	...
	pd->si = cyttsp4_request_sysinfo_(dev);
	if (!pd->si)
		return -EINVAL;

	rc = cyttsp4_setup_input_device_and_sysfs(dev);

	cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_STARTUP, CY_MODULE_PROX,
...
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp4_proximity_data *pd = &cd->pd;
	...
	if (pd->pdata->inp_dev_name)
		...
		pd->input->name = CYTTSP4_PROXIMITY_NAME;
	...
	if (pd->si) {
		...
		cyttsp4_subscribe_attention_(dev, CY_ATTEN_STARTUP,
			CY_MODULE_PROX, cyttsp4_setup_input_attention, 0);
	}
