...
static void _cyttsp4_spi_pr_buf(struct device *dev, u8 *buf,
...
#else
#define _cyttsp4_spi_pr_buf(a, b, c, d) do { } while (0)
#endif

static int cyttsp4_spi_xfer(u8 op, struct device *dev, u16 reg, u8 *buf,
	int length)
{
	...
	if (rd_hdr_buf[CY_SPI_SYNC_BYTE] != CY_SPI_SYNC_ACK) {
		...
		switch (op) {
		case CY_SPI_WR_OP:
			_cyttsp4_spi_pr_buf(dev, wr_hdr_buf,
				CY_SPI_WR_HEADER_BYTES,
				"spi_wr_buf HEAD");
			if (buf)
				_cyttsp4_spi_pr_buf(dev, buf,
					length, "spi_wr_buf DATA");
			break;

		case CY_SPI_RD_OP:
			_cyttsp4_spi_pr_buf(dev, rd_hdr_buf,
				CY_SPI_RD_HEADER_BYTES, "spi_rd_buf HEAD");
			_cyttsp4_spi_pr_buf(dev, buf, length,
				"spi_rd_buf DATA");
			break;
...
static s32 cyttsp4_spi_read_block_data(struct device *dev, u16 addr, int length,
	void *data, int max_xfer)
{
	...
	while (length > 0) {
		...
		/* Write address */
		rc = cyttsp4_spi_xfer(CY_SPI_WR_OP, dev, addr, NULL, 0);
		if (rc < 0) {
		...
		/* Read data */
		rc = cyttsp4_spi_xfer(CY_SPI_RD_OP, dev, addr, data, trans_len);
		if (rc < 0) {
...
static s32 cyttsp4_spi_write_block_data(struct device *dev, u16 addr,
	int length, const void *data, int max_xfer)
{
	...
	while (length > 0) {
		...
		rc = cyttsp4_spi_xfer(CY_SPI_WR_OP, dev, addr, (void *)data,
				trans_len);
		if (rc < 0) {
...
static int cyttsp4_spi_write(struct device *dev, u16 addr, u8 *wr_buf,
	const void *buf, int size, int max_xfer)
{
	int rc;

	pm_runtime_get_noresume(dev);
	rc = cyttsp4_spi_write_block_data(dev, addr, size, buf, max_xfer);
	pm_runtime_put_noidle(dev);

	return rc;
}

static int cyttsp4_spi_read(struct device *dev, u16 addr, void *buf, int size,
	int max_xfer)
{
	int rc;

	pm_runtime_get_noresume(dev);
	rc = cyttsp4_spi_read_block_data(dev, addr, size, buf, max_xfer);
	pm_runtime_put_noidle(dev);

	return rc;
}

static struct cyttsp4_bus_ops cyttsp4_spi_bus_ops = {
...
static int cyttsp4_spi_probe(struct spi_device *spi)
{
	...
	rc = cyttsp4_probe(&cyttsp4_spi_bus_ops, &spi->dev, spi->irq,
...
static int cyttsp4_spi_remove(struct spi_device *spi)
{
	...
	const struct of_device_id *match;
#endif
	struct cyttsp4_core_data *cd = spi_get_drvdata(spi);
...
{
	int err = spi_register_driver(&cyttsp4_spi_driver);
