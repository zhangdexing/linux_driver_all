...
enum cyttsp4_sensor_monitor_status {
	CY_MNTR_DISABLED,
	CY_MNTR_ENABLED,
};

#define CYTTSP4_DEBUG_MAX_NUMBER_OF_SENSORS	25
#define CYTTSP4_DEBUG_BYTES_PER_SENSOR		6
#define CYTTSP4_DEBUG_MAX_SENSOR_DATA_SIZE \
	(CYTTSP4_DEBUG_MAX_NUMBER_OF_SENSORS * CYTTSP4_DEBUG_BYTES_PER_SENSOR)
struct cyttsp4_sensor_monitor {
	enum cyttsp4_sensor_monitor_status mntr_status;
	u32 sensor_data_size;
	/* operational sensor data */
	u8 sensor_data[CYTTSP4_DEBUG_MAX_SENSOR_DATA_SIZE];
};
...
static int cyttsp4_xy_worker(struct cyttsp4_debug_data *dd)
{
	...
	if (dd->monitor.mntr_status == CY_MNTR_ENABLED) {
		...
		rc = cmd->read(dev, CY_MODE_OPERATIONAL,
				...
				CYTTSP4_DEBUG_MAX_SENSOR_DATA_SIZE);
		if (rc < 0)
...
static int cyttsp4_debug_cat_attention(struct device *dev)
{
	...
	if (cat_masked_cmd != CY_CMD_CAT_NULL
		...
		/* Required for signal to the TTHE */
		dev_info(dev, "%s: cyttsp4_CaT_IRQ=%02X %02X %02X\n",
...
static int cyttsp4_debug_probe(struct device *dev)
{
...
static int cyttsp4_debug_release(struct device *dev)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp4_debug_data *dd = cyttsp4_get_debug_data(dev);
