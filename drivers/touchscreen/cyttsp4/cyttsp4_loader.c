...
#define CY_FW_MANUAL_UPGRADE_FILE_NAME "cyttsp4_fw_manual_upgrade"
...
#define CYTTSP4_LOADER_FW_UPGRADE_RETRY_COUNT 3
...
#define CY_COMPUTE_CRC(buf, len) \
	_cyttsp4_compute_crc(&buf[1], len - 1)
...
static void cyttsp4_calibrate_idacs(struct work_struct *calibration_work)
{
...
static int cyttsp4_calibration_attention(struct device *dev)
{
	struct cyttsp4_loader_data *ld = cyttsp4_get_loader_data(dev);
	...
	cmd->unsubscribe_attention(dev, CY_ATTEN_STARTUP, CY_MODULE_LOADER,
		cyttsp4_calibration_attention, 0);
...
static u16 _cyttsp4_compute_crc(const u8 *buf, int size)
{
	u16 crc = 0xffff;
	u16 tmp;
	int i;
...
static int cyttsp4_add_host_sync(u8 *buf, int i)
{
...
static int _cyttsp4_ldr_enter(struct device *dev, struct cyttsp4_dev_id *dev_id)
{
	...
	i = cyttsp4_add_host_sync(ldr_enter_cmd, i);
...
static int _cyttsp4_ldr_init(struct device *dev)
{
	...
	i = cyttsp4_add_host_sync(ldr_init_cmd, i);
...
static int _cyttsp4_ldr_prog_row(struct device *dev, int row_data_idx,
		struct cyttsp4_hex_image *row_image)
{
	u16 crc;
	int next = 0;
	...
	next = cyttsp4_add_host_sync(cmd, next);
	...
	struct cyttsp4_hex_image *row_image)
{
	...
	i = cyttsp4_add_host_sync(ldr_verify_row_cmd, i);
	...
	u8 *app_chksum)
{
	...
	i = cyttsp4_add_host_sync(ldr_verify_chksum_cmd, i);
...
static int _cyttsp4_ldr_exit(struct device *dev)
{
	...
	i = cyttsp4_add_host_sync(ldr_exit_cmd, i);
...
static int _cyttsp4_load_app(struct device *dev, const u8 *fw, int fw_size)
{
	...
	u8 app_chksum;
	u8 *row_buf = NULL;
	size_t image_rec_size = sizeof(struct cyttsp4_hex_image);
	...
	while (p < (fw + fw_size)) {
		...
		/* program row */
		retval = _cyttsp4_ldr_prog_row(dev, row_data_idx, row_image);
		if (retval < 0) {
		...
		const u8 *fw_img, int fw_size)
{
	...
	int retry = CYTTSP4_LOADER_FW_UPGRADE_RETRY_COUNT;
	int rc;
		...
		bool forced)
{
	...
	if (upgrade)
		return cyttsp4_upgrade_firmware(dev, fw->img, fw->size);
		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	if (ld->loader_pdata &&
		...
		dev_vdbg(dev, "%s: Adding callback for calibration\n",
			__func__);
		rc = cmd->subscribe_attention(dev, CY_ATTEN_STARTUP,
			CY_MODULE_LOADER, cyttsp4_calibration_attention, 0);
		if (rc) {
			dev_err(dev, "%s: Failed adding callback for calibration\n",
				__func__);
			dev_err(dev, "%s: No calibration will be performed\n",
...
static int cyttsp4_get_ttconfig_version(struct cyttsp4_loader_data *ld,
		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	if (ld->loader_pdata->flags & CY_LOADER_FLAG_CHECK_TTCONFIG_VERSION) {
		...
		rc = cyttsp4_get_ttconfig_version(ld, ttconfig_data,
...
static int cyttsp4_loader_probe(struct device *dev)
{
...
#if CYTTSP4_FW_UPGRADE || CYTTSP4_TTCONFIG_UPGRADE
	init_completion(&ld->calibration_complete);
	INIT_WORK(&ld->calibration_work, cyttsp4_calibrate_idacs);
#endif
	...
	cyttsp4_fw_and_config_upgrade(&ld->fw_and_config_upgrade);
#else
...
static int cyttsp4_loader_release(struct device *dev)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp4_loader_data *ld = cyttsp4_get_loader_data(dev);
...
static int __init cyttsp4_loader_init(void)
{
	...
	for (i = 0; i < num_core_ids; i++) {
		...
		pr_info("%s: Registering loader module for core_id: %s\n",
			__func__, core_ids[i]);
		rc = cyttsp4_loader_probe(cd->dev);
		if (rc < 0) {
