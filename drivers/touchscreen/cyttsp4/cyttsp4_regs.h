...
enum cyttsp4_initialize_baselines_sensing_mode {
	CY_IB_SM_MUTCAP = 1,
	CY_IB_SM_BUTTON = 2,
	CY_IB_SM_SELFCAP = 4,
};
...
struct cyttsp4_core_data {
	...
	enum cyttsp4_fb_state fb_state;
#endif
#ifdef VERBOSE_DEBUG
...
static inline int is_crc_stat_failed(struct cyttsp4_sysinfo *si)
{
	int crc_stat;
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4_TTSP20_MODE
	if (crc_stat)
		return 1;
#else
	if (!crc_stat)
