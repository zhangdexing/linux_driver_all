...
static void cyttsp4_mt_lift_all(struct cyttsp4_mt_data *md)
{
	if (!md->si)
		return;
...
static void cyttsp4_report_event(struct cyttsp4_mt_data *md, int event,
		int value)
{
...
static void cyttsp4_get_mt_touches(struct cyttsp4_mt_data *md, int num_cur_rec)
{
	...
	for (i = 0; i < num_cur_rec; i++) {
		...
		if (tch.abs[CY_TCH_O] == CY_OBJ_HOVER) {
			cyttsp4_report_event(md, CY_ABS_D_OST,
					tch.abs[CY_TCH_P]);
			tch.abs[CY_TCH_P] = 0;
		}
		...
		for (j = 0; j <= CY_ABS_W_OST ; j++)
			cyttsp4_report_event(md, CY_ABS_X_OST + j,
					tch.abs[CY_TCH_X + j]);
		...
		if (IS_TTSP_VER_GE(si, 2, 3)) {
			...
			for (j = 0; j < CY_NUM_EXT_TCH_FIELDS; j++)
				cyttsp4_report_event(md, CY_ABS_MAJ_OST + j,
...
static int cyttsp4_mt_suspend_attention(struct device *dev)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp4_mt_data *md = &cd->md;
...
static int cyttsp4_mt_resume_attention(struct device *dev)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp4_mt_data *md = &cd->md;
...
static int cyttsp4_mt_open(struct input_dev *input)
{
	...
	cyttsp4_subscribe_attention_(dev, CY_ATTEN_IRQ, CY_MODULE_MT,
		cyttsp4_mt_attention, CY_MODE_OPERATIONAL);

	/* set up startup call back */
	cyttsp4_subscribe_attention_(dev, CY_ATTEN_STARTUP, CY_MODULE_MT,
		cyttsp4_startup_attention, 0);

	/* set up wakeup call back */
	cyttsp4_subscribe_attention_(dev, CY_ATTEN_WAKE, CY_MODULE_MT,
		cyttsp4_mt_wake_attention, 0);

	/* set up suspend call back */
	cyttsp4_subscribe_attention_(dev, CY_ATTEN_SUSPEND, CY_MODULE_MT,
		cyttsp4_mt_suspend_attention, 0);

	/* set up resume call back */
	cyttsp4_subscribe_attention_(dev, CY_ATTEN_RESUME, CY_MODULE_MT,
		cyttsp4_mt_resume_attention, 0);
...
static void cyttsp4_mt_close(struct input_dev *input)
{
	...
	cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_IRQ, CY_MODULE_MT,
		cyttsp4_mt_attention, CY_MODE_OPERATIONAL);

	cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_STARTUP, CY_MODULE_MT,
		cyttsp4_startup_attention, 0);

	cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_WAKE, CY_MODULE_MT,
		cyttsp4_mt_wake_attention, 0);

	cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_SUSPEND, CY_MODULE_MT,
		cyttsp4_mt_suspend_attention, 0);

	cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_RESUME, CY_MODULE_MT,
		cyttsp4_mt_resume_attention, 0);
...
int cyttsp4_mt_probe(struct device *dev)
{
	...
	if (md->si) {
		...
		dev_err(dev, "%s: Fail get sysinfo pointer from core p=%p\n",
			__func__, md->si);
		cyttsp4_subscribe_attention_(dev, CY_ATTEN_STARTUP,
...
int cyttsp4_mt_release(struct device *dev)
{
	...
	if (md->input_device_registered) {
		...
		input_free_device(md->input);
		cyttsp4_unsubscribe_attention_(dev, CY_ATTEN_STARTUP,
			CY_MODULE_MT, cyttsp4_setup_input_attention, 0);
	}
