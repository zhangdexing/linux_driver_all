...
#endif
	data->gpio_pin = of_get_named_gpio_flags(client->dev.of_node,
					"bma2x2,gpio_irq", 0, NULL);
	PINFO("BMA2X2 qpio number:%d\n",
					data->gpio_pin);
	err += gpio_request_one(data->gpio_pin,
					GPIOF_IN, "bma2x2_int");
	err += gpio_direction_input(data->gpio_pin);
