...
static int ub927_i2c_write(unsigned char *buf, unsigned len)
{
	int ret = -1, retries = 0;
	while(retries < 5){
		ret = SOC_I2C_Send(UB9XX_I2C_BUS, UB927_I2C_ADDR, buf, len);
		if(ret == 1)
			break;
		retries++;
		msleep(300);
...
static int ub928_i2c_write(unsigned char *buf, unsigned len)
{
	int ret = -1, retries = 0;
	while(retries < 5){
		ret = SOC_I2C_Send(UB9XX_I2C_BUS, UB928_I2C_ADDR, buf, len);
		if(ret == 1)
			break;
		retries++;
		msleep(300);
