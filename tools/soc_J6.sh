function soc_make_kernelconfig()
{
	...
	ARCH=arm make O=out ti_sdk_dra7x_android_release_defconfig
	ARCH=arm make O=out menuconfig
	cp out/.config ti_sdk_dra7x_android_release_defconfig
	rm -rf out/source
}
...
function soc_build_kernel()
{
	echo $FUNCNAME
	export ARCH=arm
	export CROSS_COMPILE=$DBG_SYSTEM_DIR/mydroid/prebuilts/gcc/linux-x86/arm/arm-linux-androideabi-4.9/bin/arm-linux-androideabi-
